...
\textbf{Uso de los comandos \texttt{cut} y \texttt{head}:}
\begin{itemize}
...
\vspace{2cm}
\textbf{Elección de los comandos:}
\begin{itemize}
...
\begin{itemize}
  ...
  \item \texttt{tail -n +2 EPIs\_cov.tsv | wc -l} $\rightarrow$ muestra a partir de la segunda fila el contenido del archivo de esta forma se elimina la primera línea (cabecera) y cuenta el número de registros restantes. En casos donde la lógica es más compleja se pueden usar herramientas mas potentes como \texttt{awk} o \texttt{sed}.
...
Se decidió agrupar los comandos dentro de llaves para generar un reporte completo en un solo paso: primero el encabezado y luego el conteo. Al encadenarlos, se asegura que el archivo \texttt{reporte\_cov.txt} quede listo con toda la información sin necesidad de editarlo manualmente después. Además, el comando \texttt{cat} al final permite verificar que el resultado es correcto.
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item \texttt{tail -n +2 reporte\_gen.txt | sort | uniq -c} $\rightarrow$ muestra a partir de la segunda fila el contenido del archivo de esta forma se elimina la primera línea que corresponde a la cabecera, ordena los valores y cuenta cuántas veces aparece cada categoría de género (Female, Male, unknown).
  \end{enumerate}
...
\begin{itemize}
  \item \texttt{cut} es la herramienta más directa para extraer columnas específicas de archivos tabulados.
  \item \texttt{tail -n +2} elimina la primera línea (cabecera), lo que evita que se mezcle con los datos reales. En casos donde la lógica es más compleja se pueden usar herramientas mas potentes como \texttt{awk} o \texttt{sed}.
  \item \texttt{sort | uniq -c} permite agrupar y contar las categorías de manera sencilla.
  \item \texttt{cat} se usa para verificar inmediatamente el contenido del archivo generado.
\end{itemize}
...
\textbf{Uso de los comandos:}
\begin{itemize}
  \item \texttt{mv -v metadata\_cov.tsv ~/Robles\_PhageOmics\_2026\_Science/data/raw/} $\rightarrow$ mueve el archivo original de metadatos a la carpeta de datos crudos (\texttt{raw}).
  \item \texttt{mv -v Generos\_cov.txt ~/Robles\_PhageOmics\_2026\_Science/data/processed/} $\rightarrow$ mueve el archivo procesado con el conteo por género a la carpeta de datos procesados.
\end{itemize}
