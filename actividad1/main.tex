...
\textbf{Introducción a la biología molecular y secuenciación} \\
\textbf{Docente: Ing. Byron Fuertes}
}
...
\section{Actividad 1: Creación del directorio de trabajo}

Para comenzar el ejercicio, lo primero fue crear el directorio raíz del proyecto.
Este directorio sigue la convención \texttt{User\_project\_year\_publication}, en mi caso:
...
La opción \texttt{-v} (verbose) muestra un mensaje confirmando la creación, lo cual ayuda a verificar que se hizo correctamente.
Luego usamos \texttt{cd} para entrar al nuevo directorio y empezar a organizar la estructura interna.

\subsection{Creación de subdirectorios y archivos}

Una vez dentro del directorio principal, organicé la estructura básica del proyecto.
Para ello, ejecuté los siguientes comandos:

\begin{minted}[fontsize=\small, frame=single, linenos]{bash}
mkdir -v analysis code data tools submission
...
El comando \texttt{touch} se usó para generar dos archivos vacíos: \texttt{README} y \texttt{LICENSE}, que servirán para documentar y dar licencia al proyecto.

\subsection{Creación de subdirectorios adicionales}

Dentro de \texttt{data}, añadí dos carpetas para diferenciar los datos en bruto y los procesados:
...
Luego, dentro de \texttt{submission}, creé dos versiones para organizar los envíos del proyecto:
...
\subsection{Verificación de la estructura con \texttt{tree}}
...
Esto permite confirmar de manera visual que los directorios \texttt{analysis}, \texttt{code}, \texttt{data}, \texttt{tools}, \texttt{submission}, y los archivos \texttt{README} y \texttt{LICENSE} se encuentran en el lugar correcto.
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=1\textwidth]{figuras/figura4_thunar.png}
    \caption{Estructura de directorios visualizados en el gestor de archivos Thunar.}
    \label{fig:figura3}
\end{figure}

\subsection{Listado de inodos y permisos}
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=1\textwidth]{figuras/figura3_permisos_inodos.png}
    \caption{Inodos y permisos de los archivos del proyecto.}
    \label{fig:figura4}
\end{figure}

Este comando combina varias opciones:
- \texttt{-R} → listado recursivo, muestra todo el contenido de subdirectorios.
- \texttt{-l} → formato detallado, incluye permisos, propietario y tamaño.
- \texttt{-h} → tamaños en formato legible (KB, MB).
- \texttt{-i} → muestra el número de inodo de cada archivo o carpeta.

De esta manera pude verificar tanto la estructura como los inodos y permisos de cada elemento del proyecto.

\subsection{Cambio de permisos del archivo README}

El archivo \texttt{README} debía tener permisos de lectura y escritura para el usuario, y solo lectura para grupo y otros.
Para lograrlo, ejecuté:
...
- \texttt{6} → usuario: lectura y escritura.
- \texttt{4} → grupo: solo lectura.
- \texttt{4} → otros: solo lectura.

Finalmente, con \texttt{ls -lh README} confirmé que los permisos se aplicaron correctamente, mostrando \texttt{-rw-r--r--}.

\subsection{Conclusión}

Con esta actividad se logró crear y organizar un directorio de trabajo completo para el proyecto.
La estructura incluye carpetas específicas para análisis, código, datos, herramientas y versiones de envío, además de los archivos \texttt{README} y \texttt{LICENSE}.

El uso de comandos como \texttt{mkdir}, \texttt{touch}, \texttt{tree}, \texttt{ls -Rlhi} y \texttt{chmod} permitió no solo construir la organización, sino también verificarla y ajustar los permisos de manera correcta.

Este tipo de organización facilita que cualquier persona (incluyéndonos a nosotros mismos en el futuro) pueda entender rápidamente qué se hizo y dónde se encuentra cada parte del trabajo.
En resumen, la actividad refuerza la importancia de documentar y mantener ordenados los proyectos de investigación desde el inicio.
